...
class VLLMEmbed:
    def __init__(
        ...
        **kwargs,
        ...
        self.pooler_config = PoolerConfig(
            pooling_type=POOL_TYPES_VLLM[pooling].upper(), use_activation=normalize
        )
    ...
    def _append_eos_token(self, texts):
        ...
        collated_texts = self.tokenizer(
            ...
            truncation=True,
            max_length=(
                self.max_length - 1 if self.append_eos_token else self.max_length
            ),
    ...
    def encode(
        ...
        **kwargs,
        ...
        for start_idx in trange(
            0, len(texts), batch_size, disable=not show_progress_bar
        ):
            sub_texts = [
                prefix + text for text in texts[start_idx : start_idx + batch_size]
            ]
            ...
            # Build vllm inputs with plain lists of token IDs
            vllm_inputs.extend(
                [
                    token_inputs(
                        prompt_token_ids=(
                            ...
                            else token_ids.tolist()
                        )
                    )
                    for token_ids in ctx_input["input_ids"]
                ]
            )
        ...
        outputs = self.model.embed(
            vllm_inputs,
            lora_request=(
                LoRARequest("emb_adapter", 1, self.lora_name_or_path)
                if self.lora_name_or_path
                else None
            ),
        )
        ...
        if self.convert_to_numpy:
            ...
            embeddings = torch.as_tensor(
                embeddings, dtype=TORCH_DTYPES["float16"], device=self.device
            )
    ...
    def encode_queries(
        ...
        return self.encode(
            queries, batch_size=batch_size, prefix=self.query_prefix, **kwargs
        )
    ...
    def encode_corpus(
        ...
        **kwargs,
        ...
        return self.encode(
            sentences, batch_size=batch_size, prefix=self.doc_prefix, **kwargs
        )
